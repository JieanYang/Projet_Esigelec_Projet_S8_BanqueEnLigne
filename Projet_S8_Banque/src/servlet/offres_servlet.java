...
public class offres_servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			ResultSet result = oDAO.getfourLastOffres();//on met dans l'objet result les 5 dernier actu grace à la fonction get... crée dans DAO
			...
			try {
				while(result.next())// on parcour le result 
				{
					...
					//pour chaque resultat trouv� on cr�e un objet offre( le resultat regroupe les 4 offres et l'objet offre représente dans l'ensemble des r�sultats une offre qui a une date un title et un text
					 offre= new Offres(result.getString("nom_service"), result.getString("description"));// méthode get de la class actualite.java
