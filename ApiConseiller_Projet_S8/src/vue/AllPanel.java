		...
		btnAjouter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (offreDAO.addOffre(tFieldtitre.getText(), tFielddesc.getText())) {// appel la fonction addoffres pour ajouter les offres dans la BDD, envoi V si ajouter et F si c'est pas le cas
					//offre = cMenu.getConseillerInfo(tFieldEmail.getText());//Si le User existe, recup鑢e les informations correspondantes en fonction de l'email utilis�
					mainFrame.setPanel(loginPanel());//appelle directement la m閠hode loginPanel() apr鑣 qu'on ai ajouter dans la BDD
				}
			}
		});
